...
class MADDPGRED:
    def __init__(self, discount_factor=0.99, tau=0.1, debug_ = False):   # discout 0.95, tau 0.01 (0.001 too small)
       ...
       #                      DDPGAgent(24, 16*8*2, 8*8*8, 2, 52, 32*8, 16*8)]
        ...
        #self.maddpg_agent = [DDPGAgent(8, 16*32, 8*32, 2, 20, 32*16, 16*16),  # 512, 256  // 512, 256 
        #                     DDPGAgent(8, 16*32, 8*32, 2, 20, 32*16, 16*16)]  # 512,1024  // 512, 256
        
        #reduced # run3
        #self.maddpg_agent = [DDPGAgent(8, 4*32, 32, 2, 20, 8*32, 16),  # 512, 256  // 512, 256 
        #                     DDPGAgent(8, 4*32, 32, 2, 20, 8*32, 16)]  # 512,1024  // 512, 256
        
        #reduced # run4
        self.maddpg_agent = [DDPGAgent(8, 8*32, 4*32, 2, 20, 16*32, 8*16),  # 512, 256  // 512, 256 
                             DDPGAgent(8, 8*32, 4*32, 2, 20, 16*32, 8*16)]  # 512,1024  // 512, 256
    ...
    def update(self, samples, agent_number, logger=None):
        ...
        n = obs_full.shape[0]
        obs_full = obs_full.reshape(n,2*8)        
        next_obs_full = torch.stack(next_obs,dim=1)
        next_obs_full = next_obs_full.reshape(n,2*8)
